...
export BOOTLOADER_MAKE_ARGS="$MAKE_ARGS BOARD=$BOARD BOOTLOADER=1 VID=$VID PID=$PID_DFU \"PRODUCT_NAME=$BOOTLOADER_NAME\" \"INTERFACE_NAME=$BOOTLOADER_NAME\""
...
env
mkdir -p $TARGET_DIR/supercan/$BOARD/01
mkdir -p $TARGET_DIR/supercan/$BOARD/02
mkdir -p $TARGET_DIR/supercan/$BOARD/03
...
for i in "$hw_revs"; do
	make $BOOTLOADER_MAKE_ARGS HWREV=$i
