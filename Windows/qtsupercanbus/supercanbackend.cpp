...
#if 0
SuperCanBackendPrivate::~SuperCanBackendPrivate()
{
    busCleanup();
}
...
SuperCanBackendPrivate::SuperCanBackendPrivate(
    ...
    , m_ChannelIndex(_channel_index)
{
//    Q_ASSERT(device.device);
//    Q_ASSERT(device.device->cmd_buffer_size);
//    Q_ASSERT(device.device->msg_buffer_size);
//    Q_ASSERT(size_t(device.cmd_tx_buffer.size()) == device.device->cmd_buffer_size);
//    Q_ASSERT(size_t(device.cmd_rx_buffer.size()) == device.device->cmd_buffer_size);

//    m_Device = std::move(device);

    Q_ASSERT(m_Device.device);
    Q_ASSERT(m_Device.device->cmd_buffer_size);
    Q_ASSERT(m_Device.device->msg_buffer_size);
    Q_ASSERT(size_t(m_Device.cmd_tx_buffer.size()) == m_Device.device->cmd_buffer_size);
    Q_ASSERT(size_t(m_Device.cmd_rx_buffer.size()) == m_Device.device->cmd_buffer_size);
...
void SuperCanBackendPrivate::expired()
{
    Q_Q(SuperCanBackend);

//    qCWarning(QT_CANBUS_PLUGINS_SUPERCAN, "Private @ %p", this);
    ...
    for (bool done = false; !done; ) {
        ...
        if (/*result >= WAIT_OBJECT_0 && */result < WAIT_OBJECT_0 + DWORD(rx_events.size())) {
            ...
            error = SC_CALL(dev_result)(m_Device.device, &transferred, &rx_ov[index], 0);
            if (error) {
                qCWarning(
                            QT_CANBUS_PLUGINS_SUPERCAN,
                            "Device %u-%u: failed to get read result: %s (%d).",
                            m_Device.index, m_ChannelIndex, SC_CALL(strerror)(error), error);
                emit q->errorOccurred(QCanBusDevice::CanBusError::ReadError);
                break;
            }
...
//                        m_TsLoUs = timestamp_us;
            ...
            qCWarning(
                        QT_CANBUS_PLUGINS_SUPERCAN,
                        "WaitForMultipleObjects failed with error %#x.",
                        GetLastError());
            emit q->errorOccurred(QCanBusDevice::CanBusError::ReadError);
            break;
        }
            ...
            if (/*result >= WAIT_OBJECT_0 && */result < WAIT_OBJECT_0 + DWORD(tx_events.size())) {
                ...
                if (error) {
                    qCWarning(
                                ...
                                m_Device.index, m_ChannelIndex, SC_CALL(strerror)(error), error);
                    emit q->errorOccurred(QCanBusDevice::CanBusError::WriteError);
                    break;
                }
            }
            else if (WAIT_TIMEOUT == result) {
        ...
        while (tx_events_available.size()) {
            auto frame = q->dequeueOutgoingFrame();
    ...
    if (!received.empty()) {
        q->enqueueReceivedFrames(received);
    }

    if (sent) {
        emit q->framesWritten(sent);
    }
}

int SuperCanBackendPrivate::setBus(bool on)
{
    if (m_IsOnBus == on) {
...
int SuperCanBackendPrivate::busOn()
{
//    qCWarning(QT_CANBUS_PLUGINS_SUPERCAN, "Private @ %p", this);

    int error = SC_DLL_ERROR_NONE;
    const size_t count = urbs <= 0 ? 16 : (urbs > MAXIMUM_WAIT_OBJECTS ? MAXIMUM_WAIT_OBJECTS : urbs);
    rx_buffer.resize(count * m_Device.device->msg_buffer_size);
    tx_buffer.resize(count * m_Device.device->msg_buffer_size);

    rx_ov.resize(count);
    tx_ov.resize(count);
    rx_events.resize(count);
    tx_events_available.resize(count);

    // clear events so in case of error we can clean up
    for (size_t i = 0; i < count; ++i) {
        rx_ov[i].hEvent = nullptr;
        tx_ov[i].hEvent = nullptr;

    }

    // create events
    for (size_t i = 0; i < count; ++i) {
        rx_ov[i].hEvent = CreateEventW(nullptr, FALSE, FALSE, nullptr);
        tx_ov[i].hEvent = CreateEventW(nullptr, FALSE, FALSE, nullptr);
        if (!rx_ov[i].hEvent || !tx_ov[i].hEvent) {
            busCleanup();
            return SC_DLL_ERROR_OUT_OF_MEM;
        }

        rx_events[i] = rx_ov[i].hEvent;
        tx_events_available[i] = static_cast<uint8_t>(i);
    }
    ...
    error = SC_CALL(dev_read)(
                ...
                reinterpret_cast<uint8_t*>(m_Device.cmd_rx_buffer.data()),
    ...
    uint8_t* cmd_tx_ptr_beg = reinterpret_cast<uint8_t*>(m_Device.cmd_tx_buffer.data());
    uint8_t* cmd_tx_ptr = cmd_tx_ptr_beg;
    memset(cmd_tx_ptr, 0, size_t(m_Device.cmd_tx_buffer.size()));
    ...
    bt->nmbt_brp = dev_to_host16(m_Device.device, nm.brp);
    bt->nmbt_sjw = nm.sjw;
    bt->nmbt_tseg1 = dev_to_host16(m_Device.device, nm.tseg1);
    bt->nmbt_tseg2 = nm.tseg2;
    bt->dtbt_brp = dt.brp;
    bt->dtbt_sjw = dt.sjw;
    bt->dtbt_tseg1 = dt.tseg1;
    bt->dtbt_tseg2 = dt.tseg2;
    ...
    size_t buffer_size = m_Device.cmd_tx_buffer.size();
    Q_ASSERT(bytes <= buffer_size);
    ...
    m_BusStatus = QCanBusDevice::CanBusStatus::Unknown;
    m_IsOnBus = true;
    connect(&m_Timer, &QTimer::timeout, this, &SuperCanBackendPrivate::expired);
    m_Timer.start();

    return error;
}

void SuperCanBackendPrivate::busOff()
{
    busCleanup();
}

bool SuperCanBackendPrivate::trySendFrame(const QCanBusFrame& frame)
{
    return trySendFrame(frame, true);
}

bool SuperCanBackendPrivate::trySendFrame(const QCanBusFrame& frame, bool emit_signal)
{
    Q_Q(SuperCanBackend);
...
void SuperCanBackendPrivate::setUrbs(int urbs)
{
    this->urbs = urbs;
}

int SuperCanBackendPrivate::setNominalBitrate(unsigned bitrate)
{
...
int SuperCanBackendPrivate::setDataBitrate(unsigned value)
{
    if (Q_UNLIKELY(!value)) {
        return SC_DLL_ERROR_INVALID_PARAM;
    }
...
int SuperCanBackendPrivate::setFd(bool value)
{
    m_Fd = value;
...
void SuperCanBackendPrivate::busCleanup()
{
    disconnect(&m_Timer, &QTimer::timeout, this, &SuperCanBackendPrivate::expired);
    m_IsOnBus = false;
    m_Timer.stop();
